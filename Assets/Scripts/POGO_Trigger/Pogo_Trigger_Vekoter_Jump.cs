...
[Tooltip("Use ForceMode.VelocityChange for more predictable, mass-independent launches.")]
public bool useVelocityChange = true;

[Tooltip("Clears existing velocity before applying the push. Prevents stacking.")]
public bool clearVelocityBeforePush = true;

[Tooltip("Clamps the resulting speed after the push to this value. 0 = no clamp.")]
...
private void ClampValues()
{
    ...
    maxLaunchSpeed = Mathf.Max(0f, maxLaunchSpeed);
}
...
private void ApplyImpulseToPlayer()
{
    ...
    if (onlyHorizontal) direction.y = 0f;
    if (moveTowardReference) direction = -direction;
...
private void DrawVector(Vector3 refPos, Vector3 playerPos, bool horizontalOnly, bool towardRef, Color color)
{
    ...
    if (towardRef) direction = -direction;
